...
class Fastqc(Tools):
    ...
    def action(self):
        ...
        self.cmds["quality controlling"] = "fastqc -t {} -o {} {} {}".format(
            self.settings.nproc, self.out, file_in1, file_in2
        )

        self.results["quality controlling"] = [
            "{}/{}_R1_fastqc.html".format(self.out, self.sample)
        ]
        self.results["quality controlling"].append(
            "{}/{}_R2_fastqc.html".format(self.out, self.sample)
        )
