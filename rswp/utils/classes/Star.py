...
class Star(Tools):
    ...
    def action(self):
        ...
        if self.build_index:
            ...
            self.cmds[
                "renaming the bam"
            ] = "mv {}/Aligned.sortedByCoord.out.bam " "{}/{}.{}".format(
                self.out, self.out, self.sample, self.settings.suffix_bam
            )

            self.results["STAR mapping"] = [
                "{}/{}.{}".format(self.out, self.sample, self.settings.suffix_bam)
            ]
    ...
    def action(self):
        if self.build_index:
            ...
            self.cmds["renaming the transcript bam"] = (
                "mv {}/Aligned.toTranscriptome.out.bam "
                "{}/{}.Transcriptome.bam".format(self.out, self.out, self.sample)
            )

            self.results["STAR mapping"].append(
                "{}/{}.Transcriptome.bam".format(self.out, self.sample)
            )
