...
class Tools:
    ...
    def runCmds(self):
        ...
        sys.stdout.write(
            "Index is {} Running {} on {}\n".format(
                self.common.index, self.name, self.sample
            )
        )
                    ...
                    if sum(a):
                        ...
                        sys.stdout.write(
                            ...
                            "if You'd Like to Rerun {} on {}\n".format(
                                " and ".join(np.array(self.results[cmd_name])[a]),
                                self.name,
                                self.sample,
                            )
                        )
    ...
    def createOut(self, name_tool_dir, out_type="dir"):
        ...
        o = "{}/{}/{}/".format(
            self.common.dir_project, name_tool_dir, self.sample
        )  # self.common.list_ids[self.common.index - 1]
        if out_type == "prefix":
    ...
    def __add__(self, dict_cmd_add):
        for key in sorted(dict_cmd_add):
            try:
                ...
                myError()
                sys.stderr.write(
                    """'{}' isn't a Correct Command Name of This Tool.\n
