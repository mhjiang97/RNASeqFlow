...
class Rsem(Tools):
    def __init__(self, fq, path_bowtie2, prepare_reference, prefix_reference, name_rsem_dir, common, settings):
...
class RsemBowtie2(Rsem):
    def __init__(self, fq, path_bowtie2, prepare_reference, prefix_reference, name_rsem_dir, common, settings):
        Rsem.__init__(self, fq, path_bowtie2, prepare_reference, prefix_reference, name_rsem_dir, common, settings)

    def action(self):
        if self.prepare_reference:
            Rsem.action(self)
            self.cmds["preparing RSEM reference"] = self.cmds["preparing RSEM reference"] + " --bowtie2 --bowtie2-path {}".format(self.path_bowtie2)
        else:
            ...
            self.cmds["RSEM quantification"] = "rsem-calculate-expression " \
                                               ...
                                               "{} {} " \
                                               "{} " \
                                               "{}/bowtie2/{}".format(self.path_bowtie2,
                                                                      self.settings.nproc,
                                                                      file_in1, file_in2,
                                                                      self.prefix_reference,
                                                                      dt, self.sample)

            self.results["RSEM quantification"] = ["{}/bowtie2/{}.genes.results".format(dt, self.sample)]
            self.results["RSEM quantification"].append("{}/bowtie2/{}.isoforms.results".format(dt, self.sample))
